...
class SWORDEntry
{
    ...
    public function buildhierarchy($sac_dr, $sac_ns)
    {
        ...
        foreach ($sac_dr->xpath("atom:link") as $sac_link) {
            $this->sac_links[] = cleanString($sac_link[0]['href']);
        }

        // Store the title and summary
        $this->sac_title = cleanString($sac_dr->children($sac_ns['atom'])->title);
        $this->sac_summary = cleanString($sac_dr->children($sac_ns['atom'])->summary);

        // Store the updated date
        $this->sac_updated = $sac_dr->children($sac_ns['atom'])->updated;

        // Store the rights
        $this->sac_rights = cleanString($sac_dr->children($sac_ns['atom'])->rights);

        // Store the format namespace
        $this->sac_packaging = $sac_dr->children($sac_ns['sword'])->packaging;

        // Store the generator
        $this->sac_generator = cleanString($sac_dr->children($sac_ns['atom'])->generator);
        ...
        $this->sac_useragent = cleanString($sac_dr->children($sac_ns['sword'])->userAgent);
