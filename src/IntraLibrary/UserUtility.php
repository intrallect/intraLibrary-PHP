...
class UserUtility
{
    ...
    public static function createUser($username, $userRole, $groupIds)
    {
        ...
        while (true) {
            try {
                return self::sendCreateUserRequest($username . $suffix, $userRole, $groupIds, true);
     ...
     */
    private static function sendCreateUserRequest($username, $userRole, $groupIds)
    {
        $req 	= new RESTRequest();
        $resp 	= $req->adminGet(
            'User/createWithGroup',
            array(
                'username' => $username,
